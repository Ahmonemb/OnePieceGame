...
local TestSkillSet = {
	["Slice"] = function(Player)
		...
		for i = 0, 2 do
			task.spawn(function()
				...
				for _ = 1, 15 do
					local Hitbox =
						workspace:GetPartBoundsInBox(NCF * CFrame.new(0, 0, i * -5), Vector3.new(2, 13, 5), Params)
					for _, Part in pairs(Hitbox) do
						if Part.Parent:FindFirstChild("Humanoid") then
							if Hits[Part.Parent] == nil then
								...
								bv.Velocity = (
									Part.Parent.HumanoidRootPart.CFrame.Position
								...
								local tween = game:GetService("TweenService")
									:Create(DmgCounter.Counter.Number, info, goal)
								tween:Play()
	...
	["Beam"] = function(Player)
		...
		BP1.Position = Character.HumanoidRootPart.Position
		BP1.MaxForce = Vector3.one * 999999
		BP1.D = 200
		BP1.P = 800
		BP1.Name = "BeamShootPosition"
		BP1.Parent = Character.HumanoidRootPart
	...
	["BeamHit"] = function(Player, ...)
		...
		for _ = 1, 30 do
			...
			local Hitbox = workspace:GetPartBoundsInBox(
				CFrame.new((Data.StartPosition + Data.EndPosition) / 2, Data.EndPosition),
				Vector3.new(4, 4, Dist),
				Params
			)
			for _, Part in pairs(Hitbox) do
				if Hits[Part.Parent] == nil then
