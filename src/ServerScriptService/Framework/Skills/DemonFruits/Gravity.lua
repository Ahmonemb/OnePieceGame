		...
		if hitPoint then
			...
			for i = 1, #targets do
				...
				SharedFunctions:FireAllDistanceClients(
					...
					script.Name,
					200,
					{ Target = target, Character = c, Function = "TargetPush" }
				)
			end
		end
	end
...
function module.Move3(p, chargeUp)
	...
	if chargeUp and chargeUp ~= "Release" then
		...
		local ray = game.Workspace:Raycast(
			c.HumanoidRootPart.Position,
			c.HumanoidRootPart.Position - Vector3.new(0, 10, 0),
			RayData
		)
		if ray then
			local partHit = ray.Instance or nil
			if partHit then
...
function module.Move4(p)
	...
	SharedFunctions:FireAllDistanceClients(
		...
		script.Name,
		200,
		{ projectileCFrame = projectileCFrame, projectileData = projectileData, Character = c, Function = "Move4" }
	)
