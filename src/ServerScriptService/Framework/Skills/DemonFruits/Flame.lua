...
function module.Move4(p)
	...
	local _ = (
		c.HumanoidRootPart:FindFirstChild("FireFliesHold")
		and c.HumanoidRootPart:FindFirstChild("FireFliesHold"):Destroy()
	)
...
--/TODO: MOVE DESCRIPTION
function module.FireFliesHold(p)
	local c = p.Character
	local cooldowns = c.Cooldowns
	...
	Val.Name = "FireFliesHold"
	Val.Parent = c.HumanoidRootPart

	while c.HumanoidRootPart:FindFirstChild("FireFliesHold") do
		task.spawn(function()
			local mousePos = getMouse:InvokeClient(p)
