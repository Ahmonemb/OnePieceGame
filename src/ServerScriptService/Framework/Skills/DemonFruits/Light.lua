...
function module.Move1(p)
	...
	SharedFunctions:FireAllDistanceClients(
		...
		script.Name,
		200,
		{ projectileCFrame = projectileCFrame, projectileData = projectileData, Character = c, Function = "Move1" }
	)
...
function module.Move2(p)
	...
	if ray then
		...
		SharedFunctions:FireAllDistanceClients(
			...
			script.Name,
			200,
			{ Magnitude = Magnitude, pos = pos, Character = c, Function = "CastMirrors" }
		)
		Root.Anchored = true
		task.wait(1.5)
		Root.Anchored = false
		...
		SharedFunctions:FireAllDistanceClients(c, script.Name, 200, {
			Shockwave = true,
...
function module.Move3(p, chargeUp)
	...
	if chargeUp and chargeUp ~= "Release" then
		...
		SharedFunctions:FireAllDistanceClients(
			...
			script.Name,
			1000,
			{ States = states, Character = c, Function = "Move3" }
		)
		return
	end
...
function module.Move4(p)
	...
	local _ = (
		c.HumanoidRootPart:FindFirstChild("LightJewelsHold")
		and c.HumanoidRootPart:FindFirstChild("LightJewelsHold"):Destroy()
	)
	--local skillData = attackData.getData(script.Name,skillName)
...
--/TODO: MOVE DESCRIPTION
function module.LightJewelsHold(p)
	local c = p.Character
	local cooldowns = c.Cooldowns
	...
	staminaHandler.checkStamina(c, "Light", skillName, true)

	local Val = Instance.new("BoolValue")
	Val.Name = "LightJewelsHold"
	Val.Parent = c.HumanoidRootPart

	while c.HumanoidRootPart:FindFirstChild("LightJewelsHold") do
		task.spawn(function()
			local mousePos = getMouse:InvokeClient(p)
			local projectileCFrame = CFrame.lookAt(
				c.HumanoidRootPart.CFrame * CFrame.new(0, 0, -1).Position,
				mousePos + Vector3.new(math.random(-5, 5), 0, math.random(-5, 5))
			)
			local points = hitDetection:GetPoints(projectileCFrame, 5, 5)
			local projectileData = {
				Points = points,
