...
local function CheckCompletion(Player)
	...
	if Data:GetAttribute("QuestProgress") >= Data:GetAttribute("QuestMax") then
		--/Give rewards
		local Rewards =
			Conversations.GetConvo(Data:GetAttribute("QuestName")).Details[Data:GetAttribute("QuestTarget")].Rewards
...
function module.IncrementQuest(Player, Target)
	...
	if Data:GetAttribute("QuestName") == "" or QuestTarget ~= Target then
		return
	end
