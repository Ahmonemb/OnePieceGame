...
local function BoatFunctionality(Boat,Data)
	...
	while Boat.Parent == Boats do wait()
		if VehicleSeat.Throttle == 1 then
			Reverse = false
			if Speed < MaxSpeed then Speed = Speed + Acceleration end
			BV.Velocity = VehicleSeat.CFrame.lookVector*Speed
		elseif VehicleSeat.Throttle == 0 then
			if not Reverse then
				if Speed <= MaxSpeed and Speed >= 0 then Speed = Speed - Acceleration*2 else Speed = 0 end
			else
				if Speed < 0  then Speed = Speed + Acceleration end
			end
			BV.Velocity = VehicleSeat.CFrame.lookVector*Speed
		elseif VehicleSeat.Throttle == -1 then
			Reverse = true
			if Speed <= MaxSpeed and Speed >= (-(MaxSpeed/4)) then Speed = Speed - Acceleration end
			BV.Velocity = VehicleSeat.CFrame.lookVector*Speed
		end
		--Steering
...
local function SpawnBoat(Player,BoatType,NPC)
	...
	coroutine.wrap(function() BoatFunctionality(Boat,Data,Boat.VehicleSeat) end)()
end
