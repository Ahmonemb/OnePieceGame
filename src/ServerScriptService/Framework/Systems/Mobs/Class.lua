...
function MobClass.new(Type, Count)
	...
	Torso.Name = "PseudoTorso"
	Torso.Anchored = false
	Torso.Massless = false
	Torso.Size = Vector3.new(2, 2, 1)
	Torso.Position = GlobalFunctions.rayCast(MobData.SpawnArea, Vector3.new(0, -30, 0), { Model }).Position
		+ Vector3.new(math.random(-25, 25), 2.9, math.random(-25, 25)) --MobData.SpawnArea+Vector3.new(math.random(-25,25),0,math.random(-25,25))
	Torso.Parent = Model
	...
	folder1.Name = "States"
	folder1.Parent = Mob.Model

	local folder2 = Instance.new("Folder")
	folder2.Name = "Data"
	folder2.Parent = Mob.Model

	for i, v in pairs(MobData.Data) do
		folder2:SetAttribute(i, v)
	end
	...
	Mob.Model:GetAttributeChangedSignal("Health"):Connect(function()
		...
		if Mob.Health <= 0 then
			...
			Header.Enabled = true
			Header.Health.Value.Text = string.format("HEALTH: %d/%d", Mob.Health, Mob.MaxHealth)
			Header.Health.Bar:TweenSize(UDim2.new((Mob.Health / Mob.MaxHealth), 0, 1, 0), "Out", "Quad", 0.6)
		end
	end)
...
local function FindNearestPlayer(self)
	for _, v in pairs(workspace:GetChildren()) do
		if
			...
			and (v.HumanoidRootPart.Position - self.Torso.Position).magnitude <= self.FollowRange
		then
			self.Target = v
		end
	end
...
function MobClass:Attack()
	...
	if CombatModule then
		...
		if self.Model.Cooldowns:FindFirstChild("Attacked") then
			return
		end
		...
		while self.Health > 0 do
			...
			if self.Target and self.Target.Humanoid.Health > 0 then
				...
				if Distance <= self.FollowRange then
					local HipHeight = GlobalFunctions.rayCast(
						self.Torso.Position,
						Vector3.new(0, -4.5, 0),
					...
					if
						HipHeight
						and not self.Model.Cooldowns:FindFirstChild("Attacked")
						and not self.Model.States:FindFirstChild("Lerping")
					then
						local Position = HipHeight.Position + Vector3.new(0, 2.9, 0)
						local TargetPosition = self.Target.HumanoidRootPart.Position
						local TargetCFrame =
							CFrame.new(Position, Vector3.new(TargetPosition.X, Position.Y, TargetPosition.Z))
						...
						if
							self.Model.Cooldowns:FindFirstChild("Attacked")
							and self.Target.States.Combo.Value < 5
							and not self.Model.States:FindFirstChild("M1KB")
						then
							print("m1")
							...
							task.wait()
						elseif
							self.Model.Cooldowns:FindFirstChild("Attacked") and self.Target.States.Combo.Value == 5
						then
							print("knockback")
						...
						elseif
							self.Model.Cooldowns:FindFirstChild("Attacked")
							and self.Target.States.Combo.Value == 6
							and not self.Model.States:FindFirstChild("SendUp")
						then
							print("sendup")
							local SendUp = Instance.new("BoolValue")
							SendUp.Name = "SendUp"
