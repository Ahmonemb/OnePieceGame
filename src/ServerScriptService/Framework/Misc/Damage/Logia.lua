...
function module.checkLogia(p, target, moveUsed)
	local demonFruit = players:GetPlayerFromCharacter(target)
			and dataStore.GetData(players:GetPlayerFromCharacter(target), "DemonFruit")
		or target.Data:GetAttribute("DemonFruit")
	if demonFruit == "" then
		return nil
	end
