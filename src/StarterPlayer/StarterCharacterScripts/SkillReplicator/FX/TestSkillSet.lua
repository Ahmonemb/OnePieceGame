...
local module = {
	["Slice"] = function(Player, Params)
		...
		for i = 0, 2 do
			...
			TS:Create(Slice, TweenInfo.new(0.5), { Transparency = 1 }):Play()
			local LastRock = time()
			...
			Connection = RS.RenderStepped:Connect(function(DT)
				...
				if Result then
					...
					Slice.Trail.Position = Vector3.new(-4.8, 0.4, Slice.Position.Y - Result.Position.Y - 0.01)
					Slice.Trail2.Position = Vector3.new(-4.8, -0.4, Slice.Position.Y - Result.Position.Y - 0.01)
					Slice.Trail3.Position = Vector3.new(-4.8, 0.4, Slice.Position.Y - Result.Position.Y - 0.005)
					Slice.Trail4.Position = Vector3.new(-4.8, -0.4, Slice.Position.Y - Result.Position.Y - 0.005)
				else
					Slice.TrailGreen.Enabled = false
					Slice.TrailBlack.Enabled = false
				end
				Slice.CFrame = Slice.CFrame * CFrame.new(-150 * DT, 0, 0)
				if time() - LastRock > 0.01 then
					local Result1 = workspace:Raycast(
					...
					if Result1 then
						...
						Rock.CFrame = CFrame.new(Result1.Position)
						...
						Rock.Size =
							Vector3.new(math.random(10, 30) / 10, math.random(10, 30) / 10, math.random(10, 30) / 10)
						Rock.Parent = workspace.FX
						Rock.Material = Result1.Material
						Rock.Color = Result1.Instance.Color
						TS:Create(Rock, TweenInfo.new(0.2), {
							CFrame = Rock.CFrame * CFrame.new(0, math.random(25, 35) / 10, 0) * CFrame.Angles(
						...
						task.delay(1, function()
							TS:Create(Rock, TweenInfo.new(0.5), {
					...
					local Result2 = workspace:Raycast(
						Vector3.new(Slice.Trail2.WorldPosition.X, Slice.Position.Y, Slice.Trail2.WorldPosition.Z),
						Vector3.new(0, -6, 0),
						RParams
					)
					if Result2 then
						local Rock = game.ReplicatedStorage.Assets.VFX.TestSkillSet.Rock:Clone()
						Rock.CFrame = CFrame.new(Result2.Position)
							* CFrame.new(math.random(-20, 20) / 10, -3, math.random(-20, 20) / 10)
							* CFrame.Angles(
								math.rad(math.random(-20, 20) / 10),
								math.rad(math.random(-20, 20) / 10),
						...
						Rock.Size =
							Vector3.new(math.random(10, 30) / 10, math.random(10, 30) / 10, math.random(10, 30) / 10)
						Rock.Parent = workspace.FX
						Rock.Material = Result2.Material
						Rock.Color = Result2.Instance.Color
						TS:Create(Rock, TweenInfo.new(0.2), {
							CFrame = Rock.CFrame * CFrame.new(0, math.random(25, 35) / 10, 0) * CFrame.Angles(
								math.rad(math.random(0, 180)),
								math.rad(math.random(0, 180)),
	...
	["Charge"] = function(Player, Params)
		...
		local Direction = Instance.new("BodyGyro")
		...
		while HRP:FindFirstChild("BeamLiftPosition") do
			if time() - WindCount > 0.2 then
				...
				TS:Create(Wind, TweenInfo.new(0.5, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
			...
			Model.CFrame = RightArm.CFrame * CFrame.new(-0.3, 0.5, 1)
		...
		repeat
			Model.CFrame = RightArm.CFrame * CFrame.new(-0.3, 1, 1)
			Direction.CFrame = CFrame.new(RightArm.Position, Mouse.Hit.Position)
			task.wait()
		until time() - StartTime > 1

		for _, Particle in pairs(Model.Attachment1:GetChildren()) do
	...
	["Beam"] = function(Player, Params)
		...
		if Player.Character == Params.Character then
			task.spawn(function()
				game.ReplicatedStorage.Remotes.Server:InvokeServer(
					"Skill",
					"None",
					{ { EndPosition = EndPos, StartPosition = StartPos }, "BeamHit", "TestSkillSet" }
				)
			end)
		end
		...
		task.spawn(function()
			for _ = 0, 10 do
				...
				TS:Create(Shock, TweenInfo.new(0.3, Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
		...
		for _, Particle in pairs(Model.Kame1.Attachment1:GetChildren()) do
			Particle.Enabled = false
		end
		for _, Particle in pairs(Model.Kame2.Attachment1:GetChildren()) do
			Particle.Enabled = false
		end

		game.Debris:AddItem(Model, 0.15)
		anim:AdjustSpeed(1)
	end,
}
