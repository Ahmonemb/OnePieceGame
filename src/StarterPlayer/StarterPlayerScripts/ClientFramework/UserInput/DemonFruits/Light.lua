...
local module = {
	...
	["Z"] = {
		["Held"] = function(p)
			...
			if cooldowns:GetAttribute("LightKick") then
				return
			end
	...
	["X"] = {
		["Held"] = function(p)
			...
			if cooldowns:GetAttribute("LightMirror") then
				return
			end
			collectionService:AddTag(c, "Aim")
			G.Aim(p, 0.3)
	...
	["C"] = {
		["Held"] = function(p)
			...
			if cooldowns:GetAttribute("LightFlight") then
				return
			end
	...
	["V"] = {
		["Held"] = function(p)
			...
			if cooldowns:GetAttribute("LightJewels") then
				return
			end
			collectionService:AddTag(c, "Aim")
			G.Aim(p, 0.3)
			...
			attackRemote:FireServer("LightJewelsHold")
		end,
		["Release"] = function(p)
			local c = p.Character
			local staminaData = attackData.getData("Light", "LightJewels")
