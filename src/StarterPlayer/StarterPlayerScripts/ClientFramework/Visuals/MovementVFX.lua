...
function module.Geppo(c)
	...
	for i = 1,4 do
		...
		smoke.CFrame = c.HumanoidRootPart.CFrame*CFrame.new(0,-1.6,0)*CFrame.Angles(math.random(360),math.random(360),math.random(360))
		smoke.Size = Vector3.new(size,size,size)
		tweenService:Create(smoke,TweenInfo.new(.35,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Transparency = 1, Size = Vector3.new(0,0,0), CFrame = smoke.CFrame*CFrame.new(math.random(-5,5),math.random(-4,0),math.random(-5,5))*CFrame.Angles(math.random(360),math.random(360),math.random(360))}):Play()
		smoke.Parent = Visual
		game.Debris:AddItem(smoke,.35)
	end
	...
	tweenService:Create(ring,TweenInfo.new(.25,Enum.EasingStyle.Quad,Enum.EasingDirection.InOut),{Transparency = 1, CFrame = ring.CFrame*CFrame.new(0,-1,0), Size = Vector3.new(15,.4,15)}):Play()
	ring.Parent = Visual
