...
function module.LightSword(c)
	...
	if not equippedWeapons:GetAttribute("LightSword") then
		...
		motor.Parent = sword
		tweenService:Create(sword, TweenInfo.new(0.1), { Transparency = 0 }):Play()
		...
		coroutine.wrap(function()
			...
			if states:GetAttribute("BusoActive") then
				...
				tweenService:Create(sword, TweenInfo.new(0.1), { Color = Color3.fromRGB(0, 0, 0) }):Play()
			end
		end)()
	else
		...
		tweenService:Create(sword, TweenInfo.new(0.1), { Transparency = 1 }):Play()
		game.Debris:AddItem(sword, 0.1)
	end
end
...
function module.CastMirrors(Data)
	...
	for i = 1, #AllCoordinates do
		...
		if Coordinate and NextCoordinate then
			local Distance1 = (Coordinate.Position - NextCoordinate.Position).Magnitude

			local GoalCFrame = CFrame.new(Coordinate.Position, NextCoordinate.Position)
				* CFrame.new(0, 0, -Distance1 / 2)
				* CFrame.fromEulerAnglesXYZ(0, math.rad(90), 0)
			local GoalSize = Vector3.new(Distance1, 1, 1)
			...
			Mirror.CFrame = CFrame.new(Coordinate.Position, NextCoordinate.Position)
...
function module.Move1(Data)
	...
	coroutine.wrap(function()
		...
		if hitPoint then
			...
			if Data.Shockwave then
				...
				coroutine.wrap(function()
					...
					local tween2 = TweenService:Create(
						...
						{ Size = Vector3.new(0, 80, 0), Color = Color3.fromRGB(255, 255, 255) }
					)
					tween2:Play()
					tween2:Destroy()
			...
			StarParticle.Star.Lifetime = NumberRange.new(0.35)
			StarParticle.Star.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 60), NumberSequenceKeypoint.new(1, 80) })
			StarParticle.Star.Transparency =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 1) })
			StarParticle.Parent = Block

			RaysParticle.Rays.Lifetime = NumberRange.new(0.5)
			RaysParticle.Rays.Color = ColorSequence.new(Color3.fromRGB(255, 255, 127))
			RaysParticle.Rays.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 100), NumberSequenceKeypoint.new(1, 0) })
			...
			RockParticle.Rock.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 2), NumberSequenceKeypoint.new(1, 0) })
			RockParticle.Rock.Transparency =
			...
			CrashSmoke.Smoke.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 20), NumberSequenceKeypoint.new(1, 20) })
...
function module.Move4(Data)
	...
	coroutine.wrap(function()
		...
		if hitPoint then
			...
			StarParticle.Star.Lifetime = NumberRange.new(0.15)
			StarParticle.Star.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 30), NumberSequenceKeypoint.new(1, 40) })
			StarParticle.Star.Transparency =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 1) })
			StarParticle.Parent = Block
			...
			RaysParticle.Rays.Color = ColorSequence.new(Color3.fromRGB(255, 255, 127))
			RaysParticle.Rays.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 40), NumberSequenceKeypoint.new(1, 0) })
			RaysParticle.Parent = Block
			...
			WaveParticle.Wave.Color = ColorSequence.new(Color3.fromRGB(255, 255, 127))
			WaveParticle.Wave.Size =
				NumberSequence.new({ NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 40) })
			WaveParticle.Parent = Block
