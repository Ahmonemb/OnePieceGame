...
function module.Crater(data)
	...
	for i = 1, data.points do
		...
		if moving then
			...
			tweenService
				:Create(
					part,
					TweenInfo.new(data.speed, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
			...
			coroutine.wrap(function()
				while moving do
					local ray = G.rayCast(
						determinedPosition + Vector3.new(0, data.size / 2, 0),
						Vector3.new(0, -10, 0),
						data.blacklist
					)
					if ray then
			...
			local ray =
				G.rayCast(determinedPosition + Vector3.new(0, data.size / 2, 0), Vector3.new(0, -10, 0), data.blacklist)
			if ray then
				part.Material = ray.Instance.Material
				part.Color = ray.Instance.Color
	...
	coroutine.wrap(function()
		task.wait((data.speed or 0) + (data.yield or 0))
