...
function LoadQuest()
	...
	if Value ~= "" then
		...
		QuestFrame.Progress.Text =
			string.format("[%d/%d]", Data:GetAttribute("QuestProgress"), Data:GetAttribute("QuestMax"))
		...
		QuestFrame.Description.Text = string
			.split(QuestData.Details[QuestTarget].Description, ".")[1]
			:format(QuestData.Details[QuestTarget].Maximum)
...
local NameConn
NameConn = Data:GetAttributeChangedSignal("QuestName"):Connect(function()
	if #HUD:GetChildren() <= 0 then
		NameConn:Disconnect()
		return
	end
	LoadQuest()
...
local ProgressConn
ProgressConn = Data:GetAttributeChangedSignal("QuestProgress"):Connect(function()
	if #HUD:GetChildren() <= 0 then
