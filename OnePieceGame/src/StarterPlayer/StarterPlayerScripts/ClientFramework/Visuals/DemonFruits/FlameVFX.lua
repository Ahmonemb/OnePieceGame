...
local WaveAttachment = script.ParticleAttatchments["WaveAttachment"]
...
function module.Move1(Data)
	...
	for i = 1,2 do
		...
		Fire.Drag = -5
		Fire.Lifetime = NumberRange.new(.25,.35)
	...
	Spark.Drag = -5
	Spark.Lifetime = NumberRange.new(.35,.5)
	...
	coroutine.wrap(function()
		...
		if hitPoint then
			...
			--[[ Fire P00rticle XD ]]--
				...
				Stars.Stars.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 1.5), NumberSequenceKeypoint.new(1, 0)}
				Stars.Stars.Drag = 5
				...
				Rocks.Rocks.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, math.random(5,10)/15), NumberSequenceKeypoint.new(1, 0)}
				Rocks.Rocks.Drag = 5
...
function module.Move2(Data)
	...
	local Fire = FireAtt.Fire
	Fire.Drag = 5
	Fire.Lifetime = NumberRange.new(.75)
	...
	FirePillar.Drag = -2
	FirePillar.Lifetime = NumberRange.new(.35,.5)
	...
	for i = 1,5 do
		...
		for j = 1,5 do
			...
			local EndPos = (Vector3.new(math.sin(360*j)*math.random(-80,80), math.tan(90*j)*math.random(10,100), math.cos(360*j)*math.random(-80,80)) + StartPos)
			...
			coroutine.wrap(function()
				for i, v in pairs(CenterAtt:GetChildren()) do
					if v:IsA("Beam") then
						...
						v.Width1 = 100
						v.Enabled = true
						local Tween = TweenService:Create(v, TweenInfo.new(.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0,  false), {["CurveSize0"] = math.random(-25,25)*i, ["CurveSize1"] = math.random(-100,100)*i, ["Width0"] = 0})		
						Tween:Play()
						Tween:Destroy()

						local Tween2 = TweenService:Create(v, TweenInfo.new(.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 0,  false), {["Width1"] = 0})		
						Tween2:Play()
						Tween2:Destroy()				
					end
				end
