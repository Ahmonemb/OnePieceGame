...
function module.ChargeUp(Data)
	...
	while Root:FindFirstChild("ExplosionCharge") do
		for i = 1,5 do
			...
			local tween2 = TweenService:Create(beam, TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {["Size"] = Vector3.new(0,0,LENGTH)})		
			tween:Play()
			tween:Destroy()
			tween2:Play()
			tween2:Destroy()
...
function module.Plant(Data)
	...
	local tween = TweenService:Create(Ball, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, 100, true, 0.25), {["Size"] = Vector3.new(3,3,3), ["Color"] = Color3.fromRGB(255, 85, 0)})
	tween:Play()
	tween:Destroy()
	...
	coroutine.wrap(function()
		...
		if hitPoint then
			...
			for _, v in ipairs(Explosion:GetChildren()) do
				...
				local tween = TweenService:Create(v, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["CFrame"] = v.CFrame * CFrame.fromEulerAnglesXYZ(math.rad(270)*i,0,0), ["Size"] = v.Size + Vector3.new(Increment,Increment,Increment)})
				tween:Play()
				tween:Destroy()
			end
...
function module.Move3(Data)
	...
	for i = 1,2 do
		coroutine.wrap(function()
			...
			Stars.Stars.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 0.5), NumberSequenceKeypoint.new(1, 0)}
			Stars.Stars.Drag = 5
			Stars.Stars.Rate = 100
			...
			local tween = TweenService:Create(Ball, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Size"] = Ball.Size * (3)})
			tween:Play()
			tween:Destroy()
			Debris:AddItem(Ball, 0.2)
	...
	SharedFunctions:BodyPosition(Root, 200, 50, Vector3.new(1e5,1e5,1e5), (Root.CFrame * CFrame.new(0,LEAP_HEIGHT,0)).Position, 0.35)
end
...
function module.Move4(Data)
	...
	--[[ Fire P00rticle XD ]]--
		...
		local tween = TweenService:Create(Ball, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Size"] = Ball.Size *(15)})
		tween:Play()
		tween:Destroy()
		Debris:AddItem(Ball, 0.2)
