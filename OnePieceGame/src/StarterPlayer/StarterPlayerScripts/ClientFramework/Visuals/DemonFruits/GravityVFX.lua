...
function module.Move1(Data)
	...
	for Index = 1,2 do
		...
		local c1,c2 = RootCFrame * CFrame.Angles(math.pi / 2,0,0) ,RootCFrame*CFrame.new(0,0,-Distance) * CFrame.Angles(math.pi/2,0,0) 
		cs.CFrame = c1
		...
		local Tween = TweenService:Create(cs,TweenInfo.new(0.35,Enum.EasingStyle.Sine,Enum.EasingDirection.Out,0,false,0),{["Transparency"] = 1, ["Size"] = Vector3.new(30,0,30), ["CFrame"] = c2})
		Tween:Play()
		Tween:Destroy()
	...
	coroutine.wrap(function()
		...
		for j = 1,60 do
			for i = 1,math.random(1,2)  do
				...
				local tween = TweenService:Create(Sphere, TweenInfo.new(.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Position"] = Sphere.Position + Vector3.new(0,-15,0)})
				tween:Play()
				tween:Destroy()
		...
		for j = 1,50 do
			for i = 1,math.random(1,2) do
				--[[ Raycast ]]--
				local StartPosition = (Vector3.new(math.sin(360 * j) * math.random(10,14) * 2, 0, math.cos(360 * j) * math.random(10,14) * 2) + RootCFrame.Position)
				...
				if ray then
					...
					if partHit then
						...
						local Tween = TweenService:Create(Block, TweenInfo.new(0.15, Enum.EasingStyle.Linear, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Orientation"] = Block.Orientation + Vector3.new(math.random(-360,360),math.random(-360,360),math.random(-360,360)), ["Position"] = Block.Position + Vector3.new(0,math.random(10,14),0)})
						Tween:Play()
						Tween:Destroy()
...
function module.Move4(Data)
	...
	local function MeteorCall()
		...
		local tween2 = TweenService:Create(Ring2, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(3,100,3)})
		tween2:Play()
		tween2:Destroy()
		...
		for i = 0,10 do
			local Ring2 = workspace.Ring2:Clone()
			Ring2.CFrame = RootCFrame * CFrame.new(0,i*10,0) * CFrame.fromEulerAnglesXYZ(math.rad(180),0,0)
			Ring2.Parent = Visual
			table.insert(Rings, Ring2)
			wait()
		end
		...
		for _, v in ipairs(Rings) do
			local tween2 = TweenService:Create(v, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Size"] = Vector3.new(0,Ring2.Size.Y,0)})
			tween2:Play()
			tween2:Destroy()
		end
		wait(0.15)
	...
	coroutine.wrap(function()
		...
		if hitPoint then
			...
			for _, v in ipairs(Explosion:GetChildren()) do
				...
				local tween = TweenService:Create(v, TweenInfo.new(.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["CFrame"] = v.CFrame * CFrame.fromEulerAnglesXYZ(math.rad(270)*i,0,0), ["Size"] = Vector3.new(0,0,0)})
				tween:Play()
				tween:Destroy()
			end
			...
			spawn(function()
				for i = 1,3 do
					...
					local tween = TweenService:Create(Ring2, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["CFrame"] =  Projectile.CFrame * CFrame.new(0,i*15,0)* CFrame.fromEulerAnglesXYZ(math.rad(180),0,0), ["Transparency"] = 1, ["Size"] = Vector3.new(75,2,75)})
					tween:Play()
					tween:Destroy()
			...
			coroutine.wrap(function()
				wait(.2)
				local tween = TweenService:Create(shock,TweenInfo.new(.2,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Size = Vector3.new(0,80,0), Color = Color3.fromRGB(255, 85, 0)})
				tween:Play()
				tween:Destroy()
			...
			Shockwave.Shockwave.Size = NumberSequence.new{NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(1, 100)}
			...
			local tween = TweenService:Create(Ball, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["Transparency"] = 1, ["Size"] = Ball.Size *(30)})
			tween:Play()
			tween:Destroy()
...
function module.Screen(Data)
	...
	local tween2 = TweenService:Create(ColorCorrection, TweenInfo.new(0.75, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["TintColor"] = Color3.fromRGB(170, 85, 255), ["Brightness"] = 0, ["Contrast"] = 0, ["Saturation"] = 0})
	tween2:Play()
	tween2:Destroy()

	wait(0.75)
	
	local tween2 = TweenService:Create(ColorCorrection, TweenInfo.new(0.75, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {["TintColor"] = Color3.fromRGB(149, 149, 149), ["Brightness"] = 0, ["Contrast"] = 0, ["Saturation"] = -1})
	tween2:Play()
	tween2:Destroy()
...
function module.TargetPush(Data)
	...
	SharedFunctions:BodyPosition(Target.HumanoidRootPart, 200, 25, Vector3.new(1e5,1e5,1e5), (Root.CFrame * CFrame.new(0,0,-75)).Position, 0.35)
