...
UIS.InputBegan:Connect(function(input,istyping)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 and Player.Character:GetAttribute("Attacking") == false and Player.Character:GetAttribute("Stunned") == false then
		HS:Attack()
	end
...
Server.OnClientEvent:Connect(function(Action)
	if Action == "PB" then
		...
		goal.Brightness = 1
		local info = TweenInfo.new(.3,Enum.EasingStyle.Sine,Enum.EasingDirection.In,2,true,0)
		...
		goal.Size = 100
		local info = TweenInfo.new(.3,Enum.EasingStyle.Sine,Enum.EasingDirection.In,2,true,0)
		local tween = game:GetService("TweenService"):Create(Blur,info,goal)
		tween:Play()
