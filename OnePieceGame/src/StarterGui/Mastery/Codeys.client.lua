...
local Conn; Conn = Remote.OnClientEvent:Connect(function(MasteryData,Action)
	if Character.Humanoid.Health <= 0 then Conn:Disconnect() return end
	if Action == "Load" then
			...
			if SkillData then
				...
				for i,v in pairs(Keys) do
					for name,move in pairs(SkillData) do
						if name ~= "MaxMastery" and move.Key == v then
							...
							MasteryCard.OpenedPosition.Value = Positions[i]
						end
					end
				end
	...
	elseif Action == "Update" then 
		for i,v in pairs(MasteryData) do 
			...
			MasteryCard.Mastery.Level.Text = string.format("MASTERY %d (MAX. %d)",v.Level,SkillData.MaxMastery)
			MasteryCard.Mastery.Experience.Text = string.format("%d/%d",v.Experience,v.MaxExperience)
			MasteryCard.Mastery.Bar:TweenSize(UDim2.new((v.Experience/v.MaxExperience),0,1,0),"Out","Linear",.45)
		end
	end
end)