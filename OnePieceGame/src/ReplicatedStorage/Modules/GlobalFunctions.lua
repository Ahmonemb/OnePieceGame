...
function module.descendantSearch(place,name)
	for i,v in pairs(place:GetDescendants()) do
		if v.Name == name then
			return v
		end
	end
...
function module.playAnim(humanoid,folderName,name,sendBack)
	local anim 
	for i,v in pairs(game.ReplicatedStorage.Assets.Animations:GetDescendants()) do
	...
	if not anim then warn(folderName.."|"..name.." does not exist.") return end
...
function module.mystCast(Orgin, Goal, Data, FilterType, IgnoreWater, CollisionGroup)
...
function module.visualizeRay(origin,goal,time,color)
	...
	part.Size = Vector3.new(.2, .2, (goal-origin).magnitude)
...
function module.getSound(name,volume)
	for i,v in pairs(game.ReplicatedStorage.Sounds:GetDescendants()) do
		if v.Name == name then
			local sound = v:Clone()
...
function module.Aim(p,delay,torque,position)
	...
	coroutine.wrap(function()
		...
		elseif collectionService:HasTag(c,"NoAim") then
			while collectionService:HasTag(c,"NoAim") do wait() end
		end
...
function module.Notify(p,text,duration)
	if p and p:IsA("Player") then
		local text = text or "<b>Couldn't get the text.</b>"
		...
		if UI then
			...
			textLabel.Position = UDim2.new(.3,0,-1.5,0)
