...
function module.damageSNG(p,target,damage,moveUsed)
	...
	--/Safezone Check 
	if target.States and target.States:GetAttribute("Safezone") then return end
	
	--/Ally Check
	if p.Allies and p.Allies:GetAttribute(target.Name) then return end
	
	--/Logia Check
	local usedLogia = logiaModule.checkLogia(p, target, moveUsed)
	if usedLogia then return true end
	...
	damageRemote:FireAllClients((target:FindFirstChild("PseudoTorso") and target.PseudoTorso.CFrame or target.HumanoidRootPart.CFrame),damage)
	if not (typeof(p) == "Instance") then return end
	...
	if TargetHealth <= 0 then
		...
		if Players:FindFirstChild(p.Name) then
			...
			elseif Players:FindFirstChild(Name) then
				...
				bounty.Value += increase
				G.Notify(Players:FindFirstChild(Name),string.format("<b>Lost<font color = 'rgb(255, 46, 46)'>%d</font> bounty.</b>",increase),2)
			end
...
function module.getGTP(p,size,cframe,singular)
	...
	part.Transparency = 1
	part.CFrame = cframe
	part.CanCollide = false part.Anchored = true
	part.Parent = Visual
	...
	for i,v in pairs(results) do
		if v.Parent and v.Parent ~= Character and (v.Parent:FindFirstChild("Humanoid") or v.Parent:FindFirstChild("PseudoTorso")) then
			if not singular then
				...
				if typeof(hitlist) ~= "Instance" then
					local AttackedHumanoidRootPart = v.Parent:FindFirstChild("PseudoTorso") and v.Parent.PseudoTorso or v.Parent.HumanoidRootPart 
